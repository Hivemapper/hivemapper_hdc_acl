...
const signature = `3drYb1k1HmnP6ubDWsVVZx4qsRBhBuWST1YsvTdnDpKhEyTgP1Nrxq53s7mMuEL7MfdQ5uWSZr3yxAsFEFKe7rH2`

const aclJsonWithFleetName = `{"managers":["3Pa4DNHKyEPJ5YQPaQBRDggstgmd89Zhr4yVMndo6T4C"],"drivers":["AW2MMchomiqbyfKUu1CkUF8n9P41H9y7C6H6MhdYkWXf"],"fleetName":"graceful-thistle-pig"}`
const storeAclWithFleetNameSignature = `48tN3wQN7ftNVzwBn5iqAVEvnkgG3cskPrNBB8PhmtCaEpQrPv6DnV4MPEKg2S4raPDYEVEqKgiHhMX4vNDz61R4`
const clearAclWithFleetNameSignature = "2TD922GcdnkkVedKeZoy9XGPzGkF7Z9erQt1z3ktGgHCUYVcxbMYXx9wwvUHqF9P2K9hGs9D7j9pngCTLQeup8gx"
...
const legacySignature = `4W13pFpEac3V2uoChfubteSQXamffosEXmBnLKLhDb2sa7ufsgKwZUteVDnr4uht83YKsyg2qhwzZwmchoz4RzTF`
...
func TestAcl_ValidateLegacySignature(t *testing.T) {
	...
	signature, err := solana.NewSignatureFromBase58(legacySignature)
	require.NoError(t, err)
...
func Test_ValidateLegacyMessageClearToSign(t *testing.T) {
	acl, err := NewAclFromData([]byte(aclJsonWithFleetName))
...
func Test_AclClearFromDeviceWithoutSignature(t *testing.T) {
	...
	acl, err := NewAclFromData([]byte(aclJsonWithFleetName))
	acl.Version = "9.9.9"
	storeSignature, err := solana.NewSignatureFromBase58(storeAclWithFleetNameSignature)
	require.NoError(t, err)

	err = acl.Store(aclFolder, storeSignature)
	require.NoError(t, err)
...
func Test_AclClearFromDeviceWithSignature(t *testing.T) {
	...
	clearSignature, err := solana.NewSignatureFromBase58(clearAclWithFleetNameSignature)
	require.NoError(t, err)

	err = AclClearFromDevice(aclFolder, clearSignature.String())
	require.NoError(t, err)
